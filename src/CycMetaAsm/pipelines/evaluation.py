...
class ContigAnalyzer:
    def __init__(self, config: EvaluationConfig) -> None:
        self.config = config
        self.evaluation_dir = Path(config.output_dir) / "evaluation" / config.assembler
    ...
    def get_contig_info(self) -> Mapping[str, Mapping[str, str]]:
        ...
        if contigs:
            self._annotate_large_contigs(contigs)
        if self.config.assembly_info:
            self._annotate_from_assembly_info(contigs)
        return contigs
    ...
    def _annotate_from_assembly_info(
        self, contigs: MutableMapping[str, MutableMapping[str, str]]
    ) -> None:
